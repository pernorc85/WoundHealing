...
class ElasticProblem 
{
  ...
  private:
    void setup_system ();
    void assemble_system (Mat_DP&, Mat_DP&, Mat_DP&);
    void solve ();
    void refine_grid ();
...
class RightHandSide :  public Function<dim> 
{
  public:
    RightHandSide ();
    RightHandSide (Mat_DP& fibroblast_density_, Mat_DP& speed_, Mat_DP& theta_);
    ...
    double fb_density(double y, double x){
        return fibroblast_density[(int)y][(int)x];  
    }
    
  protected:
    Mat_DP fibroblast_density;
    Mat_DP speed;
    Mat_DP theta;
};
...
template <int dim>
RightHandSide<dim>::RightHandSide(Mat_DP& fibroblast_density_, Mat_DP& speed_, Mat_DP& theta)
        :fibroblast_density(fibroblast_density_),speed(speed_),theta(theta_),
         Function<dim> (dim)
{}
				       ...
				       Vector<double>   &values) const 
{
  ...
  values(0) = value0*50 + 3*fb_density(p(1),p(0))*speed[(int)p(1)][(int)p(0)]/15*cos(theta[(int)p(1)][(int)p(0)]);
  values(1) = value1*50 + 3*fb_density(p(1),p(0))*speed[(int)p(1)][(int)p(0)]/15*sin(theta[(int)p(1)][(int)p(0)]);
...
{  
  ...
  RightHandSide<dim>      right_hand_side(fibroblast_density);
...
void ElasticProblem<dim>::output_results (const unsigned int cycle) const
{
  ...
  filename += std::string(itoa(cycle));
